        ...
        with open(file_path, 'r') as f:
            ...
            for s in f.read().split("END TEST"):
                ...
                try:
                    ...
                    # workaround to count final as weight index:                    
#                    if l[1] == 'final':
#                        l[1] = '51000'
                    
                    d = {'v': l[0].split('/')[-2], 'w': int(l[1]) if l[1] != 'final' else l[1], 'detections_count': int(l[3]), 'unique_truth_count': int(l[4]), 'mug_black': float(l[6])/100.0, 'mug_gray': float(l[7])/100.0, 'cocacola': float(l[8])/100.0, 'pringles': float(l[9])/100.0, 'thresh': float(l[10]), 'precision': float(l[11]), 'recall': float(l[12]), 'F1-score': float(l[13]), 'TP': int(l[15]), 'FP': int(l[16]), 'FN': int(l[17]), 'average IoU': float(l[18])/100.0, 'mAP': float(l[19])}
                    d_list += [d]
                 
                except ValueError:
                    tmp = {'v':l[0], 'w':l[1], 'detections_count': l[3], 'unique_truth_count': l[4], 'mug_black': l[6], 'mug_gray': l[7], 'cocacola': l[8], 'pringles': l[9], 'thresh': l[10], 'precision': l[11], 'recall': l[12], 'F1-score': l[13], 'TP': l[15], 'FP': l[16], 'FN': l[17], 'average IoU': l[18], 'mAP': l[19]}
...
#                    print(k, '\t', d[k])
        ...
        f_csv.write("dataset, w, detections_count, unique_truth_count, mug_black average precision, mug_gray average precision, cocacola average precision, pringles average precision, precision, recall, F1-score, TP, FP, FN, average IoU, mAP\n")
        
#        print(d_list)
        
        d_list.sort(key=lambda x: int(x['w']) if x['w'] != 'final' else 0)
        for d in d_list:
            f_csv.write(', '.join(map(str, [d['v'], d['w'], d['detections_count'], d['unique_truth_count'], d['mug_black'], d['mug_gray'], d['cocacola'], d['pringles'], d['precision'], d['recall'], d['F1-score'], d['TP'], d['FP'], d['FN'], d['average IoU'], d['mAP']])) + '\n')
